...
- **当前能力地图**(权威·现盘):genshin→`account`/`gameRegistry`/`gacha`;miao→`gameData`/`rank`;框架→`renderer`/`gamePrefix` + hook 总线 `core.hook.{on,emit}`(`emit` 为 await 异步触发)。注:`pluginRegistry`/manifest/cordis/懒激活均已删;`profile:beforeRender` 为 ark 去侵入预留 seam,埋点待 A-1 落地。

...
> ⚠️ **已被 ADR-008 取代(2026-06-02)**:本节原设计基于 cordis 3.18.1,但落地后发现 cordis 独有能力
> (Service/dispose/scope)生产中从未使用,已**去掉 cordis、改 ~40 行自管实现**(能力=Map,hook=事件总线)。
> **`core` 门面隔离的设计思想不变且被验证有效**(正因业务只依赖 `core.*`,换底座才零成本);仅"基座=cordis"
> 这一选型作废。下文 cordis 代码示例仅作历史参考,现行实现见 `lib/contracts/index.js`。
>
> ADR-001 原文:L1 基座复用 cordis 3.18.1。本节(历史)定"怎么把它挂进现有 Yunzai、且不爆改"。

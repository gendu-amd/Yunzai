...
export default class Runtime {
   ...
   */
  async getMysApi(targetType = "all", option = {}, game = false) {
    let mys = await this.getMysInfo(targetType)
    if (mys.uid && mys?.ckInfo?.ck) {
      // 第三参兼容：布尔(true=星铁，旧用法) 或 游戏 key('sr'/'zzz'/'gs')（P0-3）
      const g = game === true ? "sr" : game
      if (g) option.game = g
   ...
   * @returns {MysApi}
   */
  createMysApi(uid, ck, option = {}, game = false) {
    // 第三参兼容：布尔(true=星铁，旧用法) 或 游戏 key('sr'/'zzz'/'gs')（P0-3）
    const g = game === true ? "sr" : game
    if (g) option.game = g

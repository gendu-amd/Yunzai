...
class PluginsLoader {
  ...
  async load(isRefresh = false) {
    ...
    await Promise.allSettled(
      files.map(async file => {
        // chapter1-05 第二步：懒激活——声明了 manifest.activation 的(目录型)插件
        // 不在启动期 import，改注册"懒占位"，命中触发器才加载。详见 docs/lazy-activation-design.md。
        const act = await this.readActivation(file)
        if (act) {
          this.registerLazy(file, act)
          return
        }
        if (
          (await Bot.sleep(
    ...
    /** 统一装配:读各插件 manifest，自动 provide 能力 + 注册 gamePrefix 贡献(声明式,免样板) */
    await this.wireManifests()

    Bot.makeLog("info", `加载定时任务[${this.task.length}个]`, "Plugin")
    Bot.makeLog("info", `加载插件[${this.pluginCount}个]`, "Plugin")
    if (this.lazyCount)
      Bot.makeLog("info", `懒激活待命[${this.lazyCount}个]`, "Plugin")
  ...
  /**
   * 统一装配（ADR-007 雏形）：扫描各目录型插件的 manifest.js,据声明**自动接线**:
   *  - `provides` 为 `{能力名: () => import(...)}` 对象 → 框架自动 `core.provide`(免每插件手写 *Port 注册样板);
   *  - `contributes.gamePrefix: [{game,test,cmd}]` → 注册到框架 gamePrefix(游戏自己声明前缀,框架只装配);
   *  - manifest 本身注册到 pluginRegistry。
   * 单一声明源 = manifest;框架据此装配。向后兼容:provides 为数组(旧声明)时不自动 provide(插件自注册照旧)。
   */
  async wireManifests() {
    const core = globalThis.Bot?.core
    if (!core) return
    let dirs
    try {
      dirs = (await fs.readdir(this.dir, { withFileTypes: true })).filter(d => d.isDirectory())
    } catch {
      return
    }
    for (const d of dirs) {
      const rel = `${this.dir}/${d.name}/manifest.js`
      if (!(await Bot.fsStat(rel))) continue
      try {
        const mod = await import(`../../${rel}`)
        const m = mod.default ?? mod.manifest
        if (!m) continue
        /** 注册 manifest(幂等) */
        core.manifest?.register?.(m)
        /** 自动 provide:provides 为对象 {name: importer} 时 */
        if (m.provides && !Array.isArray(m.provides)) {
          for (const [name, importer] of Object.entries(m.provides)) {
            if (typeof importer !== "function" || core.has(name)) continue
            try {
              const impl = await importer()
              core.provide(name, impl?.default ?? impl)
              Bot.makeLog("mark", `[contracts] 自动装配能力 [${logger.green(name)}] ← ${d.name}`, "Plugin")
            } catch (err) {
              Bot.makeLog("warn", [`自动装配能力失败 [${name}] ← ${d.name}`, err], "Plugin")
            }
          }
        }
        /** 注册 gamePrefix 贡献 */
        if (Array.isArray(m.contributes?.gamePrefix) && core.gamePrefix?.register)
          for (const gp of m.contributes.gamePrefix) {
            try {
              core.gamePrefix.register({ game: gp.game, test: new RegExp(gp.test), cmd: gp.cmd })
              Bot.makeLog("mark", `[contracts] 装配游戏前缀 [${gp.game}] ← ${d.name}`, "Plugin")
            } catch (err) {
              Bot.makeLog("warn", [`装配游戏前缀失败 ← ${d.name}`, err], "Plugin")
            }
          }
      } catch (err) {
        Bot.makeLog("warn", [`manifest 装配失败 ${logger.red(d.name)}`, err], "Plugin")
      }
    }
  }

  /**
   * 懒激活：读取(目录型)插件 manifest.js 的 activation 声明触发器。
   * 仅 目录型(index.js)插件支持；无 manifest 或无 activation → 返回 null(走 eager)。
   * manifest.js 约定零副作用、不 import 重模块(见 docs/lazy-activation-design.md)。
   * @returns {Promise<null|{matchers:RegExp[], priority:number}>}
   */
  async readActivation(file) {
    if (!file?.name || file.name.includes("/")) return null
    const rel = `${this.dir}/${file.name}/manifest.js`
    if (!(await Bot.fsStat(rel))) return null
    try {
      const mod = await import(`../../${rel}`)
      const m = mod.default ?? mod.manifest
      const act = m?.activation
      if (!act || (!act.prefix?.length && !act.regex?.length)) return null
      const matchers = [
        ...(act.prefix || []).map(p => new RegExp("^" + lodash.escapeRegExp(p))),
        ...(act.regex || []).map(r => new RegExp(r)),
      ]
      if (!matchers.length) return null
      return { matchers, priority: act.priority ?? m.priority ?? 100 }
    } catch (error) {
      Bot.makeLog("warn", [`懒激活 manifest 读取失败 ${logger.red(file.name)}`, error], "Plugin")
      return null
    }
  }

  /** 懒激活：注册一个"懒占位"到 priority(无 plugin/class，命中触发器才真正加载) */
  registerLazy(file, act) {
    this.priority.push({
      lazy: true,
      key: file.name,
      file,
      matchers: act.matchers,
      priority: act.priority,
      name: file.name,
    })
    this.lazyCount = (this.lazyCount || 0) + 1
    Bot.makeLog("debug", `懒激活待命 [${file.name}]`, "Plugin")
  }

  /**
   * 懒激活：派发期命中触发器→import 真实插件→真实 rule 进 priority→删占位→重排序。
   * 命中后最终路由仍走插件真实 rule，与 eager 等价。仅激活一次。
   */
  async activateLazy(e) {
    const hits = this.priority.filter(i => i.lazy && i.matchers.some(r => r.test(e.msg)))
    if (!hits.length) return
    const keys = new Set(hits.map(h => h.key))
    // 先删懒占位，避免并发/重入重复激活
    this.priority = this.priority.filter(i => !(i.lazy && keys.has(i.key)))
    for (const h of hits) {
      Bot.makeLog("mark", `懒激活 [${logger.green(h.key)}]`, "Plugin")
      await this.importPlugin(h.file)
      // 正确性兜底：懒加载插件若含 accept/task/handler(激活前应已存在的能力)，告警建议改 eager
      for (const it of this.priority)
        if (
          it.key === h.key &&
          !it.lazy &&
          (it.plugin?.accept || it.plugin?.task?.length || it.plugin?.handler)
        ) {
          Bot.makeLog(
            "warn",
            `懒激活插件 [${h.key}] 含 accept/task/handler，激活前可能缺失行为，建议改 eager`,
            "Plugin",
          )
          break
        }
  ...
  async loadPlugin(file, p) {
    ...
    /** R-3 单次实例化(ADR-006)：复用 init 后的同一实例承载 rule/handler，
     *  不再 new 第二个(消除 init 副作用丢失 + 双构造开销)。执行期仍 new i.class(e)。 */
    /** 初始化正则表达式 */
    if (plugin.rule)
   ...
   */
  /**
   * 顶层派发边界(R-1 / ADR-006)：await + try/catch 防 unhandled rejection + 耗时 tracing。
   * 派发本体见 `_deal`(逻辑与原 deal 逐字一致)。后续 R-2 将 `_deal` 拆为中间件管道。
   */
  async deal(e) {
    const _t = Date.now()
    try {
      return await this._deal(e)
    } catch (err) {
      Bot.makeLog("error", [`${e?.logText || ""}[派发异常]`, err], false)
    } finally {
      Bot.makeLog("debug", `${e?.logText || ""}[派发耗时${Bot.getTimeDiff(_t)}]`, false)
    }
  }
  ...
  async _deal(e) {
    ...
    // 判断游戏（原神/星铁/绝区零），标准化命令前缀。
    // 必须在 Runtime.init 之前执行，确保 e.user 解析 uid/ck 时 e.game 已确定（P0-1）。
    // 三态安全 setter：置 false 时只在当前正是该游戏时才回退到 gs，
    // 避免把 zzz 误冲成 gs/sr（P0-2）。gs/sr 场景与旧行为一致。
    Object.defineProperty(e, "isSr", {
      get: () => e.game === "sr",
      set: v => {
        if (v) e.game = "sr"
        else if (e.game === "sr") e.game = "gs"
      },
    })
    ...
    Object.defineProperty(e, "isZzz", {
      ...
      set: v => {
        if (v) e.game = "zzz"
        else if (e.game === "zzz") e.game = "gs"
      },
    })
    // chapter2 A-3：前缀→游戏 归一化优先走框架自有 gamePrefix 注册表(权威、可扩展)，
    // 取不到才回退本地硬编码 srReg/zzzReg(行为逐字一致)。详见 lib/contracts/gamePrefixRegistry.js。
    const detected = globalThis.Bot?.core?.gamePrefix?.detect?.(e.msg)
    if (detected) {
      e.game = detected.game
      e.msg = detected.msg
    } else if (this.srReg.test(e.msg)) {
      e.game = "sr"
    ...
    /** 懒激活：命中触发器的插件此刻加载进 this.priority(见 docs/lazy-activation-design.md) */
    await this.activateLazy(e)

    await this._mwDispatch(e)
  }

  /**
   * 派发核心(R-2 / ADR-006)：构建生效 priority(跳过懒占位) → getContext 上下文 →
   * only_reply_at 门 → accept → rule 正则匹配(首匹配执行即结束)。
   * 逻辑与原 _deal 末段逐字一致,抽成独立方法便于单测/插入横切关注点。其中 return 等价于结束派发。
   */
  async _mwDispatch(e) {
    const priority = []
    for (const i of this.priority) {
      /** 跳过未激活的懒占位(无 plugin/class) */
      if (i.lazy) continue
      /** 判断是否启用功能，过滤事件 */

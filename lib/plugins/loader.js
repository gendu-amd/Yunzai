...
class PluginsLoader {
  ...
  async _deal(e) {
    ...
    // 前缀→游戏 归一化走框架 gamePrefix 注册表(游戏前缀由各游戏插件在自己 index 直接 register,
    // 框架不硬编码游戏;详见 lib/contracts/gamePrefixRegistry.js)。
    const detected = globalThis.Bot?.core?.gamePrefix?.detect?.(e.msg)
    if (detected) {
      e.game = detected.game
      e.msg = detected.msg
    }
    // e.game 单一事实源:无前缀命中则默认原神(gs),保证 e.game 恒有值。
    // 这是多游戏判定的承重不变量 —— 下游据此逐步去掉各自的 `e.game || (e.isSr?'sr':'gs')` 兜底
    // (isSr/isGs/isZzz 仅作派生 getter/setter,不再作为独立判定源)。
    if (!e.game) e.game = "gs"
  ...
  /**
   * 派发核心(R-2 / ADR-006)：构建生效 priority → getContext 上下文 →

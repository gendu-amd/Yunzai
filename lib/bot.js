...
import coreContract from "./contracts/index.js"
...
export default class Yunzai extends EventEmitter {
  ...
  // R-4(ADR-006)：adapter 按 id 幂等注册——已存在同 id 则"替换而非追加"，
  // 消除热更/重复 import 导致的 Bot.adapter 叠加(§2.1.5)。对各 adapter 透明(仍调 .push)。
  adapter = Object.assign([], {
    push(...items) {
      for (const it of items) {
        const i = it?.id ? this.findIndex(a => a?.id === it.id) : -1
        if (i >= 0) {
          this[i] = it
          logger?.debug?.(`[adapter] 重复注册 [${it.id}] → 替换(R-4 幂等)`)
        } else {
  ...
  // L1 契约层挂载：Bot.core 门面(provide/require/has/list + hook)，供 provider/consumer 发现。
  // 也可直接 import "lib/contracts/index.js" 拿同一单例。(ADR-008 后无 cordis ctx,去掉 Bot.ctx)
  core = coreContract

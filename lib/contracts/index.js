...
/**
 * 框架级游戏命令前缀注册表 `gamePrefix`（chapter2 A-3）。框架自有 + 默认项(sr/zzz)，
 * 正则与 loader 旧 srReg/zzzReg 逐字一致、顺序一致(sr→zzz)。插件可 register 新游戏。
 */
const gamePrefix = createGamePrefixRegistry()
gamePrefix
  .register({ game: "sr", test: /^#?(\*|星铁|星轨|穹轨|星穹|崩铁|星穹铁道|崩坏星穹铁道|铁道)+/, cmd: "#星铁" })
  .register({ game: "zzz", test: /^#?(%|％|绝区零|绝区)+/, cmd: "#绝区零" })
